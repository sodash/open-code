 ...
 *
 */
public class Img0Servlet extends HttpServlet {
	...
	public static final String COOKIE_DOMAIN = ".soda.sh";
	...
	public static String getCreateCookieTrackerId(WebRequest state) {
		...
		WebUtils2.addCookie(state.getResponse(), "trkid", uid, TUnit.YEAR.dt, COOKIE_DOMAIN);
		return uid;
	}
	...
	private void processIncoming(WebRequest state) {	
		...
		try {
			File file = new File("web/img/tracking-pixel.gif");
			assert file.isFile();
			FileServlet.serveFile(file, state);
		} catch (IOException e) {
			Log.w("img0", e);
		}
		// Count it
		String grp = state.getSubDomain();		
		String cref = null;
		if (ref!=null) {
			// remove gumpf??
...
//	static final SField INTRO = new SField("intro");
