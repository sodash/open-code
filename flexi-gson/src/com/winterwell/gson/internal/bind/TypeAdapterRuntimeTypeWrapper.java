...
final class TypeAdapterRuntimeTypeWrapper<T> extends TypeAdapter<T> {
	...
	public void write(JsonWriter out, T value) throws IOException {
		...
		if (runtimeType != type) {
			TypeAdapter runtimeTypeAdapter = context.getAdapter(TypeToken
					.get(runtimeType));
			if ( ! (runtimeTypeAdapter instanceof ReflectiveTypeAdapter/*ReflectiveTypeAdapterFactory.Adapter*/)) {
				// The user registered a type adapter for the runtime type, so
				// we will use that
				chosen = runtimeTypeAdapter;
			} else if ( ! (delegate instanceof ReflectiveTypeAdapter/*ReflectiveTypeAdapterFactory.Adapter*/)) {
