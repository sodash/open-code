...
package com.winterwell.gson.stream;
...
public class JsonReader implements Closeable {
	...
	/**
	 * Returns the next token, a {@link com.winterwell.gson.stream.JsonToken#NAME
	...
	/**
	 * Returns the {@link com.winterwell.gson.stream.JsonToken#STRING string} value
	...
	/**
	 * Returns the {@link com.winterwell.gson.stream.JsonToken#BOOLEAN boolean}
	...
	/**
	 * Returns the {@link com.winterwell.gson.stream.JsonToken#NUMBER double} value
	...
	public double nextDouble() throws IOException {
		...
		if (result > Long.MAX_VALUE && ! Double.isInfinite(result) && ! Double.isNaN(result) && peekedString.indexOf('.') == -1
				&& peekedString.indexOf('E') == -1) {
			BigInteger bi = new BigInteger(peekedString);
			result = bi.doubleValue();
		}
		if (!lenient && (Double.isNaN(result) || Double.isInfinite(result))) {
			throw new MalformedJsonException(
					"JSON forbids NaN and infinities: " + result + " at line "
	...
	/**
	 * Returns the {@link com.winterwell.gson.stream.JsonToken#NUMBER double} or
	...
	/**
	 * Returns the {@link com.winterwell.gson.stream.JsonToken#NUMBER long} value of
	...
	/**
	 * Returns the {@link com.winterwell.gson.stream.JsonToken#NUMBER int} value of
	 * the next token, consuming it. If the next token is a string, this method
		...
		if (p == PEEKED_LONG) {
			...
			if (peekedLong != result) { // Make sure no precision was lost
				...
				throw new NumberFormatException("Expected an int but was "
						+ peekedLong + " at line " + getLineNumber()
	...
	private int nextNonWhitespace(boolean throwOnEof) throws IOException {
		...
		if (throwOnEof) {
			throw new EOFException("End of input" + " at line "
					+ getLineNumber() + " column " + getColumnNumber());
		} else {
			return -1;
		}
	...
	private IOException syntaxError(String message) throws IOException {
		throw new MalformedJsonException(message + " at line "
				+ getLineNumber() + " column " + getColumnNumber() + " path "
				+ getPath());
	}
