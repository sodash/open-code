 ...
 * 
 * <pre>
 * com.winterwell.gson.JsonParseException: Expecting object found: "(5,6)"
 *   at com.winterwell.gson.JsonObjectDeserializationVisitor.visitFieldUsingCustomHandler
 *   at com.winterwell.gson.ObjectNavigator.navigateClassFields
 *   ...
 * </pre>
 *
...
public final class MapTypeAdapterFactory implements TypeAdapterFactory {
	...
	public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> typeToken) {
		...
		TypeAdapter<?> valueAdapter = gson.getAdapter(TypeToken
		...
		TypeAdapter<T> result = new Adapter(gson, keyAndValueTypes[0],
	...
	private final class Adapter<K, V> extends TypeAdapter<Map<K, V>> {
		...
		public Adapter(Gson context, Type keyType,
				TypeAdapter<K> keyTypeAdapter, Type valueType,
				TypeAdapter<V> valueTypeAdapter,
				ObjectConstructor<? extends Map<K, V>> constructor) 
		{
			this.gson = context;
			this.keyTypeAdapter = new TypeAdapterRuntimeTypeWrapper<K>(context,
					keyTypeAdapter, keyType);
			this.valueTypeAdapter = new TypeAdapterRuntimeTypeWrapper<V>(
		...
		public void write(JsonWriter out, Map<K, V> map) throws IOException {
			...
			// Loop check
			boolean ok = out.beginLoopCheck(gson.getLoopPolicy(), map);
			if (!ok) {
				out.nullValue();
				return;
			}
			...
			for (Map.Entry<K, V> entry : map.entrySet()) {
				JsonElement keyElement = keyTypeAdapter.toJsonTree(entry
						.getKey());
				keys.add(keyElement);
				values.add(entry.getValue());
				hasComplexKeys |= keyElement.isJsonArray()
						|| keyElement.isJsonObject();
			}
