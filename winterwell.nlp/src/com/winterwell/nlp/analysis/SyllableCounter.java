...
/**
 * ?? How does this compare with http://www.syllablecount.com/syllable/rules/ ??
 * 
 * Note: The PHP adapted code (commented out below) claims to be more accurate,
 * but something has gone wrong in my port-to-Java.
 * 
 * <p>
...
public class SyllableCounter {
	private static final Pattern[] AddSyl = new Pattern[] {
			...
			Pattern.compile("([^aeiouy])\1l$"), // middle twiddle battle bottle,
												// etc.
			Pattern.compile("[^l]lien"), // alien, salient [1]
			Pattern.compile("^coa[dglx]."), // [2]
			Pattern.compile("[^gq]ua[^auieo]"), // i think this fixes more than
												// it
												// breaks
	...
	/*
	 * basic algortithm: each vowel-group indicates a syllable, except for:
	 * final (silent) e 'ia' ind two syl @AddSyl and @SubSyl list regexps to
	 * massage the basic count. Each match from @AddSyl adds 1 to the basic
	 * count, each
	 * 
	 * @SubSyl match -1 Keep in mind that when the regexps are checked, any
	 * final 'e' will have been removed, and all '\'' will have been removed.
	 */
	 ...
	 */
	public static final int syllableCount(final String _word) {
		if (_word == null || "".equals(_word))
			return 0;
		
		// punctuation?
		if (StrUtils.isPunctuation(_word)) {
			return 0;
		}
		
		final String word = StrUtils.toCanonical(_word).replaceAll("'", "")
				.replaceAll("e$", "");
		
		// one letter (after pruning e)
		if (word.length() == 1) {
			return 1;
		}
		
		final String[] scrugg = word.split("[^aeiouy]+"); // '-' should be
															// perhaps
															// added?
...
//
// /**
// * A syllable counter, adapted from the TextStatistics class
...
// // Array of problem words (with word : key, syllable count : value)
// ArrayMap<String,Integer> arrProblemWords = new ArrayMap<String,Integer>(
// "simile" , 3
// ,"forever" , 3
// ,"shoreline" , 2
// );
//
// // These syllables would be counted : two but should be one
// String[] arrSubSyllables = new String[]{
// "cial"
// ,"tia"
// ,"cius"
// ,"cious"
...
// String[] arrAddSyllables = new String[]{
// "ia"
// ,"riet"
// ,"dien"
// ,"iu"
...
// String[] arrPrefixSuffix = new String[]{
// "^un"
// ,"^fore"
// ,"ly$"
// ,"less$"
...
// * @param strWord
// */
// public int syllableCount(String strWord) {
...
// for(String prefSuff : arrPrefixSuffix) {
// Matcher m = Pattern.compile(prefSuff).matcher(strWord);
// while(m.find()) {
// prefixSuffixCount++;
// }
// strWord = m.replaceAll("");
// }
//
...
// intSyllableCount = intWordPartCount + prefixSuffixCount;
// for(String strSyllable : arrSubSyllables) {
// if (Pattern.compile(strSyllable).matcher(strWord).find()) {
// intSyllableCount --;
// }
// }
...
// }
// }
// if (intSyllableCount == 0) intSyllableCount = 1;
